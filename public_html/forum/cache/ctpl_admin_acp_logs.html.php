...
<?php if (sizeof($this->_tpldata['log'])) {  ?>
	...
	<?php $_log_count = (isset($this->_tpldata['log'])) ? sizeof($this->_tpldata['log']) : 0;if ($_log_count) {for ($_log_i = 0; $_log_i < $_log_count; ++$_log_i){$_log_val = &$this->_tpldata['log'][$_log_i]; if (!($_log_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } ?>
			...
			<td style="text-align: center;"><a href="<?php echo $_log_val['U_SHOW_IP']; ?>"><?php echo $_log_val['IP']; ?></a><br />[ <a href="<?php echo $_log_val['U_WHOIS']; ?>" onclick="popup(this.href, 700, 500, '_whois'); return false;"><?php echo ((isset($this->_rootref['L_WHOIS'])) ? $this->_rootref['L_WHOIS'] : ((isset($user->lang['WHOIS'])) ? $user->lang['WHOIS'] : '{ WHOIS }')); ?></a> ]<br />[ <a href="<?php echo $_log_val['U_RECHECK_SPAM']; ?>" onclick="popup(this.href, 700, 500, '_whois'); return false;"><?php echo ((isset($this->_rootref['L_RECHECK_SPAM'])) ? $this->_rootref['L_RECHECK_SPAM'] : ((isset($user->lang['RECHECK_SPAM'])) ? $user->lang['RECHECK_SPAM'] : '{ RECHECK_SPAM }')); ?></a> ]</td>
